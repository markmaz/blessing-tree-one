...
let count = ref(0);let giftCount = ref(0);

async function fetchFamilies(){
  try{
    const response = await giftService.outstandingCount();
    ...
    <template #content>
      ...
      >
        ...
        <div class="item item-rounded-lg bg-body-light">
          <i class="fa fa-gifts fs-3 text-danger"></i>
        </div>
      </div>
      <div class="bg-body-light rounded-bottom">
        <RouterLink :to="{name: 'backend-gifts'}" class="block-content block-content-full block-content-sm fs-sm fw-medium d-flex align-items-center justify-content-between">
          <span>View all Outstanding Gifts</span>
